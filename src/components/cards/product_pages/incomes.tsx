...
// import { getIncomes } from 'store/reducers/income';
...
const Incomes = ({ color = 'primary', title, count, percentage, isLoss, router, value, id }: Props) => {
  ...
  const GoDeltail = (env: any) => {
    console.log("点击", 'account' + (Number(env.target.id) - 2).toString())
    // const params = {
    //   tra_id: 'account' + (Number(env.target.id) - 2).toString(),
    //   type: 'account',
    //   token: getToken().token

    // }
    // dispatch(getIncomes(params))
  }
