...
const NavGroup = ({ item, lastItem, remItems, lastItemId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) => {
  ...
  const checkOpenForParent = (child: NavItemType[], id: string) => {
    child.forEach((ele: NavItemType) => {
      ...
      if (ele.url === pathname) {
        const params = {
          account_id: Number(ele.url.charAt(ele.url.length - 1)),
          type: 'account',
          token: getToken().token
        }
        dispat(getAccount(params))
        console.log('ele.url', ele.url.charAt(ele.url.length - 1))
  ...
  const checkSelectedOnload = (data: NavItemType) => {
    ...
    childrens.forEach((itemCheck: NavItemType) => {
      ...
      if (itemCheck.url === pathname) {
        // const params = {
        //   prod_id: itemCheck.url.charAt(itemCheck.url.length - 1),
        //   type: 'account',
        //   token: getToken().token
        // }
        // dispat(getAccount(params))
        // console.log('ele.url', itemCheck.url.charAt(itemCheck.url.length - 1))
